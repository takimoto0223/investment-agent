...
実行タイミング：
  ...
  06:00 → generate_morning_report(report_ctx, daytrade_pl, daytrade_candidates)
...
"""
    ...
    def _build_morning_extras(
        ...
        daytrade_candidates: list[DaytradeCandidate] | None,
        usdjpy: float,
    ) -> dict:
        """
        朝レポート専用の追加データを組み立てる。
          ...
          - daytrade_candidates: ScalpDay_JP スクリーニング結果（None 可）
        """
        # ── デイトレ損益 ──────────────────────────────────────────────
        ...
        return {
            ...
            "daytrade_candidates": daytrade_candidates or [],
...
<p style="color:#888;font-size:12px;">
  ...
  {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
    ...
    def generate_morning_report(
        ...
        daytrade_candidates: list[DaytradeCandidate] | None = None,
    ) -> bool:
        """
        ...
        extras = self._build_morning_extras(
            logs, daytrade_pl, daytrade_candidates, report_ctx.usdjpy_rate
        )
        ...
        report_data = MorningReportData(
            ...
            daytrade_candidates=extras["daytrade_candidates"],
            ...
            value_decisions=extras["value_decisions"],
        )
